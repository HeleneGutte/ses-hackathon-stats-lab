...
We will use data from the [`palmerpenguins`](https://github.com/allisonhorst/palmerpenguins) package for the first few sections [@R-palmerpenguins]. We download the data set from github using the remotes package [@remotes]. 
...
(ii) Are there common patterns among the states (species)? <br>
...
Above you see two different visualizations of the same variable (there are several ways to arrange two or more plots together, above you see a simple way to specify that both plots should be arrange together in two columns using the ggpubr package [@ggpubr]). On the left a **boxplot** and on the right a **histogram**. Think about which one you like more and why? What are possible advantages of each visualisation?
...
We are using the tidyverse syntax in the following. You could do the same calculations using base R (see the example in the code), it is just a matter of convenience... 
...
Explore PCA, kmeans, and clustering below using the `vegan` package [@vegan] and a data set it contains called `dune`. For an in-depth tutorial on multivariate analysis and ordinal methods with the `vegan` R packages, see [this vignette](https://www.mooreecology.com/uploads/2/4/2/1/24213970/vegantutor.pdf)
...
To plot dendrograms we use the package factoextra [@factoextra].
